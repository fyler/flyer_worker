...
-define(COMMAND(In, OutName),
  "gs -dNOPAUSE -dBATCH -dSAFER -sDEVICE=png16m -dFirstPage=1 -dLastPage=1 -sOutputFile=\""
  ++OutName++"\" -r15 -q \""++In++"\" -c quit"
).
...
run(#file{tmp_path = Path, name = Name, dir = Dir},_Opts) ->
  ...
  Data = exec_command:run(?COMMAND(Path, Out)),
  ?D({gs_data,Data}),
  case  filelib:is_file(Out) of
    true ->
      {ok, #job_stats{time_spent = ulitos:timestamp() - Start, result_path = [list_to_binary(Name ++ "_thumb.png")]}};
    _ ->
      {error, {pdf_thumb_failed, Data}}
  end.
