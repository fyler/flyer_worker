...
run(#file{name = Name, dir = Dir} = File, Opts) ->
  ...
  case split_pdf:run(File, Opts) of
    {ok, #job_stats{result_path = [PDF]}} -> case pdf_to_pages:run(#file{tmp_path = filename:join(Dir,binary_to_list(PDF)), name = Name, dir = Dir}) of
                                               {ok, #job_stats{result_path = Pages}} -> case pdf_to_thumbs:run(#file{tmp_path = filename:join(Dir,binary_to_list(PDF)), name = Name, dir = Dir}) of
